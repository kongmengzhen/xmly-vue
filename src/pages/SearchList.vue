  ...
  <div class="index-warp">
    <div class="header">
      <header>
        ...
        <a href="javascript:0;" class="searchInput">
          <form action="#" 
          class="header-form"  
          @click="searchHandle"      
          >
       ...
       <div class="tab-list">
        <ul class="tab-ul">
             <li               
                ...
                @click="addActive(i,tabitem.paramUrl)" 
              >{{tabitem.tabnm}}</li>          
        </ul>
      </div>       
    </div>   
    <div class="allWarp">
      ...
      <div class="allResultList"  
      v-if="showAll"  
      >     
        <div class="resultContent">
          <h2 class="title"
          v-if="albumsObj.trackViews.tracks.length>0"
          >
            <span>"{{seatchKy}}"相关的主播</span>
            <a class="moreText">
              <span class="moreText">更多               
              </span>
               <i class="yo-ico goMore">&#xe602;</i>
            </a>
          </h2>
          <ul class="userList">           
            <searchCicle
            class="user-item"
            :userDetail="albumsObj"
            ></searchCicle>
          </ul>          
         </div>
        <div class="resultContent">
            ...
            <ul >  
              <searchColitem 
                class="searchColitem"
                 v-for="albumitem in albumsObj.albumViews.albums"
                :key="albumitem.albumInfo.id"
                @click="jumpDetail"
                :detail="albumitem">
              </searchColitem>            
            </ul>          
         ...
         <div class="resultContent">
          ...
          <ul class="trackList">                
            <a class="track-item" 
              v-for="trackitem in albumsObj.trackViews.tracks"
              :key="trackitem.trackInfo.id"
            >
            <img class="track-cover" :src="trackitem.trackInfo.cover_path" alt="">
            <div>
              <p class="trackTitle">{{trackitem.trackInfo.title}}</p>
              <p class="trackInfo">209.93万</p>
            </div>
            </a>
          </ul>          
        </div>
        <div class="resultContent">
          <h2 class="title">
           ...
           <ul >  
             <specialColitem
              :detail="specialitem"
              v-for="specialitem in albumsObj.specialViews.specials"
              :key="specialitem.id"
             >            
             </specialColitem>
...
<script>
 ...
 import searchColitem from "components/index/searchColitem";
 import specialColitem from "components/index/specialColitem";
 import searchCicle from "components/index/searchCicle"
 import {get} from 'untils/http.js'
  import Vue from 'vue'
  export default { 
    data(){
      return{
        seatchKy:this.$route.params.id,
        currentIndex:"",
        showAll:true, 
        albumsObj:{},
        tabList:[
    ...
    methods: {
      ...
      async addActive(i,paramUrl){
        this.currentIndex=i
        console.log(paramUrl);   
        if(paramUrl){  
          this.showAll=false    
          this.$router.push('/search/'+paramUrl+'/'+this.seatchKy);
        }else{
          this.showAll=true
          this.$router.push('/search/'+this.seatchKy);          
      }
          paramUrl=paramUrl==='album'?'album':(paramUrl==='sound'?'track':(paramUrl==='zhubo'?'user':(paramUrl==='ting'?'special':'all')))        
      let result= await get({
      url:'https://m.ximalaya.com/m-revision/page/search?kw='+this.seatchKy+'&core='+paramUrl+'&page=1&rows=15'
      })    
      this.albumsObj=result.data  
   ...
   async mounted() {
      let result=  await get({
        url:'https://m.ximalaya.com/m-revision/page/search?kw='+this.seatchKy+'&core=all&page=1&rows=5'
      })
      this.albumsObj=result.data  
     
      this.$nextTick(()=>{
        let bScroll=new BScroll('.allWarp',{
          pullUpLoad:true,
          click:true,
          probrType:2,
          mouseWheel:true,
